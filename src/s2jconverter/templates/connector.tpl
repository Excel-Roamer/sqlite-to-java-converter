...
public class Connector {
    ...
    public ArrayList<String> selectOneColumn(String table, String column, String whereClause) {
        try {
            ArrayList<String> data = new ArrayList();
            ResultSet rs;
            qf.setTableName(table);
            rs = mcon.prepareStatement(qf.getSelectQuery(new String[] {column}, whereClause)).executeQuery();
            ResultSetMetaData rsmd = rs.getMetaData();
            int l = rsmd.getColumnCount();
            while (rs.next()) {
    ...
    public HashMap<String, ArrayList> execute(String query) {
        try {
            HashMap<String, ArrayList> data = new HashMap();
            ResultSet rs;
            rs = mcon.prepareStatement(query).executeQuery();
            ResultSetMetaData rsmd = rs.getMetaData();
            int l = rsmd.getColumnCount();
            for (int i = 1; i <= l; i++) {
                data.put(rsmd.getColumnName(i), new ArrayList());
